...
def pad_arbitrary_tensors(tensors, padding_value=0.0):
    max_len = torch.stack([torch.tensor(x.shape) for x in tensors]).max(dim=0)[0]
    padded_tensor = torch.empty(
        [len(tensors)] + max_len.tolist(), device=tensors[0].device
    ).fill_(padding_value)
...
class Spatial_ImageList(object):
    ...
    def from_tensors(
        ...
        if size_divisibility > 1:
            ...
            max_size[-2:] = (max_size[-2:] + (stride - 1)).div(
        ...
        if len(tensors) == 1:
            ...
            padding_size = [
                0,
                max_size[-1] - image_size[2],
                0,
                max_size[-2] - image_size[1],
            ]
