...
class GeneralizedXdecoder(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        loss_weights = {
            ...
            "retrieval": {
                "decoder": dec_cfg["RETRIEVAL_WEIGHT"],
                "backbone": dec_cfg["BACKBONER_WEIGHT"],
            },
            "grounding": {
    ...
    def forward_seg(self, batched_inputs):
        ...
        for key, value in outputs.items():
            if key == "pred_logits":
                ...
                _outputs[key] = value[:, : self.num_queries - 1]
                if self.task_switch["grounding"]:
                    _outputs["pred_gmasks"] = value[
                        :, self.num_queries : 2 * self.num_queries - 1
                    ]
            elif key == "pred_captions":
                _outputs[key] = value[:, : self.num_queries - 1]
                if self.task_switch["grounding"]:
                    _outputs["pred_gtexts"] = value[
                        :, self.num_queries : 2 * self.num_queries - 1
                    ]
                ...
                for i in range(len(value)):
                    ...
                    for _key, _value in value[i].items():
                        if _key == "pred_logits":
                            ...
                            _outputs[key][i][_key] = _value[:, : self.num_queries - 1]
                            if self.task_switch["grounding"]:
                                _outputs[key][i]["pred_gmasks"] = _value[
                                    :, self.num_queries : 2 * self.num_queries - 1
                                ]
                        elif _key == "pred_captions":
                            _outputs[key][i][_key] = _value[:, : self.num_queries - 1]
                            if self.task_switch["grounding"]:
                                _outputs[key][i]["pred_gtexts"] = _value[
                                    :, self.num_queries : 2 * self.num_queries - 1
                                ]
        outputs = _outputs
    ...
    def forward_vlp(self, batched_inputs):
        ...
        if self.task_switch["retrieval"] and self.retrieval_emsemble:
            ...
            loss_contrast = image_text_contrastive_loss_queue(
                v_emb, t_emb, self.sem_seg_head.predictor.lang_encoder, None
            )
    ...
    def prepare_vlp_targets(self, batched_inputs, device):
        ...
        for cnt, x in enumerate(batched_inputs):
            ...
            target_dict["caption_tokenids"] = lang_results["tokens"]["input_ids"][
                cnt : cnt + 1
            ]
            target_dict["caption_mask"] = lang_results["tokens"]["attention_mask"][
                cnt : cnt + 1
            ]
    ...
    def prepare_targets(self, batched_inputs, images):
        ...
        for idx, batch_per_image in enumerate(batched_inputs):
            ...
            if self.task_switch["caption"]:
                ...
                noun_captions = [
                    prompt_engineering(noun, topk=10000, suffix=".") for noun in nouns
                ] + [text]

                self.sem_seg_head.predictor.lang_encoder.get_text_embeddings(
                    noun_captions, is_eval=False, name="caption_noun", prompt=False
                )
                ctext = getattr(
                    self.sem_seg_head.predictor.lang_encoder,
                    "{}_text_embeddings".format("caption_noun"),
                )
                target_dict["captions"] = ctext

                target_dict["captions_hash"] = [
                    (hash(st.stem(txt)) % 10**16) for txt in (nouns + [text])
                ]
                target_dict["labels_hash"] = [
                    (
                        hash(
                            st.stem(
                                COCO_PANOPTIC_CLASSES[label_id]
                                .replace("-other", "")
                                .replace("-merged", "")
                                .replace("-stuff", "")
                            )
                    ...
                    for label_id in target_dict["labels"]
                ]
