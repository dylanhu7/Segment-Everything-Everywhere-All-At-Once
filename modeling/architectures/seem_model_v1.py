...
class GeneralizedSEEM(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        loss_weights = {
            "mask": {
                "ce": dec_cfg["CLASS_WEIGHT"],
                "dice": dec_cfg["DICE_WEIGHT"],
                "bce": dec_cfg["MASK_WEIGHT"],
            },
            "bbox": {"l1": dec_cfg["BBOX_WEIGHT"], "giou": dec_cfg["GIOU_WEIGHT"]},
            "spatial": {
                "ce": dec_cfg["SCLASS_WEIGHT"],
                "dice": dec_cfg["SDICE_WEIGHT"],
                "bce": dec_cfg["SMASK_WEIGHT"],
            },
            "grounding": {
                "ce": dec_cfg["GCLASS_WEIGHT"],
                "dice": dec_cfg["GDICE_WEIGHT"],
                "bce": dec_cfg["GMASK_WEIGHT"],
            },
            "openimage": {
                "ce": dec_cfg["OCLASS_WEIGHT"],
                "dice": dec_cfg["ODICE_WEIGHT"],
                "bce": dec_cfg["OMASK_WEIGHT"],
            },
        }
        ...
        spatial_cost = {
            "class_weight": dec_cfg["COST_SPATIAL"]["CLASS_WEIGHT"],
            ...
            if turn_on:
                if isinstance(loss_weights[key], dict):
                    ...
                    for key_, weight in loss_weights[key].items():
                        weight_dict["loss_{}_{}_0".format(key, key_)] = (
                            weight  # NOTE: hard code for segmentation that has multiple loss
                        )
                else:
        ...
        grd_weight = {
            "text": dec_cfg["GROUNDING"]["TEXT_WEIGHT"],
            "class": dec_cfg["GROUNDING"]["CLASS_WEIGHT"],
        }
        ...
        train_dataset_name = cfg["DATASETS"]["TRAIN"][
            0
        ]  # HACK for only one training set.
    ...
    def forward(self, batched_inputs, mode="default"):
        ...
        if self.training:
            losses = {}
            if (
                self.task_switch["mask"]
                or self.task_switch["grounding"]
                or self.task_switch["spatial"]
            ):
    ...
    def forward_seg(self, batched_inputs):
        ...
        if "instances" in batched_inputs[0]:
            ...
            if self.task_switch["spatial"]:
                ...
                fp_masks = nn.utils.rnn.pad_sequence(
                    [
                        (x["spatial_query"]["rand_shape"].to(self.device) & False)
                        for x in batched_inputs
                    ],
    ...
    def evaluate_interactive(self, batched_inputs):
        ...
        if self.interactive_mode in ["best", "best_random"]:
            pos_masks = [
                x["spatial_query"]["rand_shape"].to(self.device)[:, 0]
                for x in batched_inputs
            ]
            ...
            neg_masks = [
                (x["spatial_query"]["rand_shape"].to(self.device) & False)[:, 0]
                for x in batched_inputs
            ]
            ...
            pos_masks = (
                batched_inputs[0]["spatial_query"]["rand_shape"].to(self.device) == 1
            ).unbind(0)
            ...
            extra.update(
                {
                    "spatial_query_pos_mask": pos_masks[:, 0:1].unbind(),
                    "spatial_query_neg_mask": neg_masks[:, 0:1].unbind(),
                }
            )
        else:
        ...
        for i in range(self.interactive_iter):
            ...
            pred_smask = F.interpolate(
                outputs["prev_mask"], images.tensor.shape[-2:], mode="bilinear"
            )
            ...
            if (ious > 0.9).sum() == len(ious):
                all_batch_shape_iou += [
                    ious for j in range(self.interactive_iter - i - 1)
                ]
                break
            if self.interactive_mode in ["best", "best_random"]:
                extra.update(
                    self.prepare_next_spaital_mask(
                        extra, batched_inputs, mode=self.interactive_mode
                    )
                )
            elif self.interactive_mode == "random":
                extra.update(
                    {
                        "spatial_query_pos_mask": pos_masks[:, i + 1 : i + 2].unbind(),
                        "spatial_query_neg_mask": neg_masks[:, i + 1 : i + 2].unbind(),
                    }
                )
            else:
                assert False, "invalid interactive mode"
    ...
    def prepare_next_spaital_mask(self, outputs, batched_inputs, mode="best"):
        ...
        gt_masks = Spatial_ImageList.from_tensors(
        ...
        prev_masks = nn.utils.rnn.pad_sequence(
            outputs["spatial_query_pos_mask"], padding_value=False, batch_first=True
        ) | nn.utils.rnn.pad_sequence(
            outputs["spatial_query_neg_mask"], padding_value=False, batch_first=True
        )
        ...
        iou = (gt_masks & pred_masks).sum(list(range(2, len(fn.shape)))) / (
            (gt_masks | pred_masks).sum(dim=list(range(2, len(fn.shape)))) + 1e-8
        )
        ...
        if mode == "best":
            max_xy_idx = torch.stack(
                [torch.arange(bs * ns), mask_dt.max(dim=-1)[1].cpu()]
            ).tolist()
            ...
            max_xy_idx = torch.stack(
                [
                    ...
                    torch.cat(
                        [
                            (mask_dt[i] > 0).nonzero()[
                                torch.randint(0, len((mask_dt[i] > 0).nonzero()), (1,))
                            ][0]
        ...
        dilation = 3
        next_mask = (
            F.conv2d(next_mask, self.dilation_kernel, padding=dilation // 2).reshape(
                bs, ns, h, w
            )
            > 0
        )
        ...
        for idx, ip in enumerate(is_postive):
            ...
            pos_mask += [
                outputs["spatial_query_pos_mask"][idx]
                | (next_mask[idx][:mask_len] & ip[:mask_len, None, None])
            ]
            neg_mask += [
                outputs["spatial_query_neg_mask"][idx]
                | (next_mask[idx][:mask_len] & (~ip[:mask_len, None, None]))
            ]
    ...
    def prepare_targets4query(self, targets, images, topk=5):
        ...
        for targets_per_image in targets:
            ...
            unique_target_classes = [
                k for k in set(targets_per_image.gt_classes.tolist())
            ]
            selected_target_classes = random.sample(
                unique_target_classes, min(topk, len(unique_target_classes))
            )
            ...
            for clss in selected_target_classes:
                ...
                self.sem_seg_head.predictor.lang_encoder.get_text_embeddings(
                    [COCO_PANOPTIC_CLASSES[clss]], name="grounding"
                )
                query = getattr(
                    self.sem_seg_head.predictor.lang_encoder,
